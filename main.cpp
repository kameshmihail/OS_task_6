...
int main(void) {
    ...
    std::vector<int> notrespondingsids;
    int exit = 0;
    ...
    while(!exit) {
        ...
        switch (command[comId]) {
            ...
            for (auto i = allids.begin(); i!= allids.end(); ++i) {
                ...
                to.send(*lists[listID]);
                if(!from.recvCheck(*lists[listID])) {
                    notrespondingsids.push_back(*i);
                }
            }
            if (notrespondingsids.empty()) {
                std::cout << "OK: -1" << std::endl;
            }
            else {
                for (auto i = notrespondingsids.begin(); i!= notrespondingsids.end(); ++i) {
                    std::cout << *i;
                    std::cout << " ";
                }
                std::cout << std::endl;
                notrespondingsids.clear();
            }
            break;
